...
const Detail = ({currentUser, setEditing }) => {
  ...
  const onSubmit = data => {
    ...
    const fileRef = storageRef.child(`wishlist/${data.image[0].name}`);
    ...
    // Firebaseのデータをupdateする!!

    firebase.firestore().collection("wishlist").doc(id).update({
      id: user.id,
      // image: user.image,
      image: `https://firebasestorage.googleapis.com/v0/b/digiqlo-554a3.appspot.com/o/wishlist%2F${data.image[0].name}?alt=media`,
      label: user.label,
      color: user.color,
      year: user.year,
      brand: user.brand,
